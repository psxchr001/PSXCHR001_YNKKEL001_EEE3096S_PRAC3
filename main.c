...
int main(void)
{
  ...
  while (1)
  {
	 ...
	 HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);
	  ...
	  HAL_Delay (1000/DELAY); // wait for 500 ms
...
uint32_t pollADC(void){
	...
	status =  HAL_ADC_PollForConversion(&hadc, 1000);
	if (status == HAL_OK){
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	float div = adc_val / 4096.0;
	float scaled = div*47999;
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, (int) scaled);
	return scaled;
}
